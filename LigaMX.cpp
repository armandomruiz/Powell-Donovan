...
void matches(std::vector<Equipo> &Equipos){
    for(int i = 0; i < Equipos.size(); i++){
        for(int j = i+1; j < Equipos.size();j++){
            ...
            std::cout << Equipos[i].m_nombre << " " << n << " - " << m << " " << Equipos[j].m_nombre << "\n";
...
void liguilla(std::vector<Equipo> &Equipos){
    ...
    for(int j = 11, k = 18; j < 15, k >14; j++, k--){
        ...
        std::cout << "Partido de ida: " << Equipos[j].m_nombre << " " << n << " - " << m << " " << Equipos[k].m_nombre << "\n";
        calcula_puntos(n,m,&Equipos[j],&Equipos[k]);
        n = poisson(Equipos[k].m_promedio_goles);
        m = poisson(Equipos[j].m_promedio_goles);
        std::cout << "Partido de vuelta: " << Equipos[k].m_nombre << " " << n << " - " << m << " " << Equipos[j].m_nombre << "\n";
    ...
    std::cout << "Las semifinales quedarian asi: " << "\n";
    for(int i = 0; i < semis.size(); i++)
        std::cout << semis[i].m_nombre << "\n";
    /*semifinales */
    reset(semis);
    ...
    for(int j = 0, k = 3; j < 2, k > 1; j++, k--){
        ...
        std::cout << "Partido de ida: " << semis[j].m_nombre << " " << n << " - " << m << " " << semis[k].m_nombre << "\n";
        calcula_puntos(n,m,&semis[j],&semis[k]);
        n = poisson(semis[k].m_promedio_goles);
        m = poisson(semis[j].m_promedio_goles);
        std::cout << "Partido de vuelta: " << semis[k].m_nombre << " " << n << " - " << m << " " << semis[j].m_nombre << "\n";
    ...
    std::cout << "La final seria: " << "\n";
    std::cout << final[0].m_nombre << " vs " << final[1].m_nombre << "\n";
    /*final */
    ...
    if(n < m){
        std::cout << "Final: " << final[0].m_nombre << " " << n << " - " << m << " " << final[1].m_nombre << "\n";
        std::cout << "El campeon es: " << final[1].m_nombre << "\n";
        for(int i = 0; i < Equipos.size(); i++){
        ...
        std::cout << "Final: " << final[0].m_nombre << " " << n << " - " << m << " " << final[1].m_nombre << "\n";
        std::cout << "El campeon es: " << final[0].m_nombre << "\n";
        for(int i = 0; i < Equipos.size(); i++){
...
void ordenar(std::vector<Equipo> &equipos){
    ...
    int n = equipos.size();
    for(int i = n-1; i >= 0; i--) 
        std::cout << "Equipo: " << equipos[i].m_nombre << ". Puntos: " << equipos[i].m_puntos << "\n";
}
...
int main() {
    ...
    std::cout << "\n";
    for(int i = 0; i < Equipos.size(); i++){
        if(Equipos[i].m_titulos != 0)
            Equipos[i].print_datos();
    }
    return 0;
}